        ...
        <div class="col-lg-5">
          ...
          @if (Cart::instance("wishlist")->content()->where('id',$product->id)->count()>0)
              <form method="POST" action="{{ route('wishlist.item.remove', ['rowId' => Cart::instance('wishlist')->content()->where('id', $product->id)->first()->rowId]) }}" id="frm-remove-item">
                @csrf
                @method('DELETE')
                <a href="javascript:void(0)" class="menu-link menu-link_us-s add-to-wishlist filled-heart" onclick="getEementById('frm-remove-item').submit();"><svg width="16" height="16" viewBox="0 0 20 20"
            ...
            <div class="swiper-slide product-card">
              <div class="pc__img-wrapper">
                ...
                <img loading="lazy" src="{{ asset('uploads/products')}}/{{$rproduct->image}}" width="330" height="400" alt="{{$rproduct->name}}" class="pc__img">
                    @foreach (explode(",", $product->images) as $gmig)                        
                      <img loading="lazy" src="{{asset('uploads/products')}}/{{$gmig}}" width="330" height="400" alt="Cropped Faux leather Jacket" class="pc__img pc__img-second">
                    @endforeach
                </a>
